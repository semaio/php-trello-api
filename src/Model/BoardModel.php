 ...
 * @codeCoverageIgnore
 */
class BoardModel extends AbstractModel implements BoardModelInterface
{
     ...
     * {@inheritdoc}
     */
    public function setOrganization(OrganizationModelInterface $organization)
    {
    ...
    {
        return new OrganizationModel($this->client, $this->client->getOrganizationApi(), $this->getOrganizationId());
    }
    ...
    public function getLists()
    {
        ...
        foreach ($this->data['lists'] as $data) {
            $lists[$data['id']] = new CardListModel($this->client, $this->client->getCardListApi(), $data['id']);
        }
     ...
     * {@inheritdoc}
     */
    public function getList(string $idOrName): CardListModel
    {
        ...
        throw new InvalidArgumentException(sprintf('There is no list with name or id "%s" on this board ("%s")', $idOrName, $this->getName()));
    }
