 ...
 * @codeCoverageIgnore
 */
class MemberModel extends AbstractModel implements MemberModelInterface
{
    ...
    {
        if (!in_array($memberType, ['admin', 'normal', 'observer'], true)) {
            throw new InvalidArgumentException(sprintf('The member type %s does not exist.', $memberType));
        }
    ...
    public function getBoards()
    {
        ...
        foreach ($this->data['idBoards'] as $boardId) {
            $boards[] = new BoardModel($this->client, $this->client->getBoardApi(), $boardId);
        }
    ...
    public function getOrganizations()
    {
        ...
        foreach ($this->data['idOrganizations'] as $organizationId) {
            $organizations[] = new OrganizationModel($this->client, $this->client->getOrganizationApi(), $organizationId);
        }
